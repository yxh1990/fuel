...
function($, _, i18n, Backbone, utils, models, commonViews, dialogViews, networkTabTemplate, networkTemplate, rangeTemplate, networkingParametersTemplate, networkTabVerificationControlTemplate) {
    ...
    NetworkTab = commonViews.Tab.extend({
        ...
        calculateButtonsState: function() {
            ...
            //this.networkConfiguration.validationError 这个是backbone中对models的验证方法的返回值
            // models.NetworkConfiguration 中的validate函数
            var errors = false;
        ...
        prepareIpRanges: function() {
            ...
            this.networkConfiguration.get('networks').each(function(network) {
                ...
                //修改meta里面的cidr和networks中设置的cidr一致
                /*if(network.get('meta').cidr && network.get("name")=="public")
                {
                    console.log("meta cidr is:"+network.get('meta').cidr);
                    console.log("network cidr is:"+network.get('cidr'));
                    console.log(network.get('meta'))
                    network.get('meta')['cidr']=network.get('cidr');
                    console.log(network.get('meta'))
                }*/
                //提交到后台后meta信息没有进行修改.只修改了net_groups的cidr
            });
